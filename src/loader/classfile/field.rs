...
pub struct Field {
    ...
    attributes: Array<attribute::FieldAttribute>
}

impl Parse<Field> for Field {
    fn parse(buf: &mut BinaryReader) -> Result<Field, ParseError> {
...
fn field_err(err: ParseError) -> ParseError {
    ParseError::new(format!("bad field: {err:?}"))
}
